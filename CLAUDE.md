...
- **The agent contract is un-collapsible to a boolean.** `toAgentVerdict` (`lib/agent/verdict.ts`, served by `GET /api/verify`, wrapped by the CLI `bin/rwa-verify.ts` and the MCP server `mcp/server.ts`) must never emit a `safe`/`is_safe` boolean. Backing is two axes — `tier` and `confidence` — and `caveats` must be non-empty unless (`tier === verified_backed` AND `confidence === verified`). An agent acting at machine speed must be forced to see the caveat; that is the whole point. The web card, CLI, and MCP tool are all clients of this one server-side contract.
